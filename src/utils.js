...
export const calculateTier = (win, pick, ban) =>
  (parseFloat(win) +
    ((parseFloat(win) * parseFloat(pick)) / 5 +
      (parseFloat(win) * parseFloat(ban)) / 5)) *
  100;
  ...
  return {
    headerClassName: (params) => {
      ...
      if (
        !currSortColumn ||
        !currSortColumn[0] ||
        params.field !== currSortColumn[0].field
      ) {
        return baseClass;
      }
      return clsx(
        `${baseClass} active-sorting-cell active-sorting-cell-${currSortColumn[0].sort}`
      );
    },
...
export const convertStatToLineGraph = (positionRanks, statToGet) => {
  ...
  positionRanks.map((stat) => {
    ...
    dataSet[stat.updateDate] = {
      ...dataSet[stat.updateDate],
      [stat.position]: getFloat(stat[statToGet]),
    };
  });

  const xData = Object.keys(dataSet)
    .sort(
      (a, b) => DateTime.fromISO(a).toMillis() - DateTime.fromISO(b).toMillis()
    )
    .slice(-10); // SHOW ONLY LAST 10 ITEMS
  const pickOptions = xData.map((date) =>
    DateTime.fromISO(date, { zone: "UTC+8" }).toFormat("d LLL y")
  );
...
export const convertStatToAppearPie = (positionRanks) => {
  ...
  positionRanks.map((stat) => {
    ...
    dataSet[stat.updateDate] = {
      ...dataSet[stat.updateDate],
      [stat.position]: getFloat(stat["appear_rate"]),
    };
  });

  const xData = Object.keys(dataSet).sort(
  ...
  const totalPick = Object.values(appearData).reduce(
    (prev, curr) => prev + curr
  );
